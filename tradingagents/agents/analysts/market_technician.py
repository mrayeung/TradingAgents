...
Use get_stock_data first for price/volume history, then get_indicators ONLY for the few confirmation indicators you genuinely need (e.g. vwma, atr, boll/boll_ub/boll_lb for structure; cci for cyclical deviation) using their exact names, and pass look_back_days=180 (about six months) so the structural and cyclical reads have enough macro context — do not reproduce the Market Analyst's full indicator sweep. For macro/breadth/intermarket items without a live data feed (Fed path, yield curve, credit spreads, A/D line, McClellan), reason explicitly from established relationships and clearly label those parts as qualitative.
