...
Errors extend TradingDesk's VendorError hierarchy so the routing layer
handles them correctly:
  - Missing key  → VendorNotConfiguredError (silently skip to next vendor)
  - Rate limit   → VendorRateLimitError     (silently skip to next vendor)
  - Other errors → VendorError              (logged, skip to next vendor)
"""
...
# Kept for backwards-compat with any code that catches FinnhubUnavailableError.
class FinnhubUnavailableError(VendorNotConfiguredError):
...
def get_news_finnhub(ticker: str, start_date: str, end_date: str) -> str:
    ...
    except (FinnhubUnavailableError, VendorNotConfiguredError, VendorRateLimitError):
        raise
    except Exception as e:
        status = getattr(getattr(e, "response", None), "status_code", None)
        if status == 429:
            raise VendorRateLimitError(f"Finnhub rate-limited for {ticker}") from e
        logger.warning("Finnhub company news error for %s: %s", ticker, e)
        raise VendorError(f"Finnhub company news failed: {e}") from e
...
def get_global_news_finnhub(
    ...
    try:
        ...
        status = getattr(getattr(e, "response", None), "status_code", None)
        if status == 429:
            raise VendorRateLimitError("Finnhub rate-limited for global news") from e
        logger.warning("Finnhub global news error: %s", e)
        raise VendorError(f"Finnhub global news failed: {e}") from e


# ---------------------------------------------------------------------------
# Social sentiment  (Reddit + StockTwits aggregate)
# ---------------------------------------------------------------------------
...
def get_social_sentiment_finnhub(
    ...
    Raises:
        VendorNotConfiguredError: if plan doesn't include this endpoint (403).
        VendorError: on other failures.
    """
    try:
        client = _get_client()
        ...
        twitter = data.get("twitter", [])   # Finnhub labels StockTwits data as "twitter"
        ...
        if status == 403:
            raise VendorNotConfiguredError(
                f"Finnhub social sentiment requires a paid plan (403 for {ticker}). "
                f"Upgrade at https://finnhub.io/pricing"
            ) from e
        if status == 429:
            raise VendorRateLimitError(f"Finnhub rate-limited for social sentiment {ticker}") from e
        logger.warning("Finnhub social sentiment error for %s: %s", ticker, e)
        raise VendorError(f"Finnhub social sentiment failed: {e}") from e
